...
public class MetroUtilsTest {
    ...
    private static final String SplitPoint = "不限";
    ...
    public void parse() throws IOException {
        ...
        breadthFirst("徐家汇", stationMap, firstFunctions);
        breadthFirst("漕河泾开发区", stationMap, firstFunctions);
    }
    ...
    private Map<String, List<Double>> getSamples(Collection<Station> stations) {
        ...
        for (Station s : stations) {
            samples.put(s.name, allSamplesForStation(String.format("http://sh.lianjia.com%sl2", s.link)));
        }
        return samples;
    }

    private List<Double> allSamplesForStation(String url) {
        try { Thread.sleep(100); } catch (InterruptedException e) {}
        List<Double> samples = new ArrayList<Double>();
        if (null == url || 0 == url.length()) {
            return samples;
        }
        HtmlResource resource = new HtmlDownloader().download(new HtmlTask(url));
        Elements all = Jsoup.parse(resource.asString()).children();
        Elements select = all.select("ul#house-lst").select("div.price").select("span.num");
        for(Element e : select){
            samples.add(Double.valueOf(e.text()));
        }
        samples.addAll(allSamplesForStation(getNextPageUrl(all)));
        return samples;
    }

    private String getNextPageUrl(Elements all) {
        for (Element a : all.select("[class=page-box house-lst-page-box]").select("a")) {
    ...
    private String getComplateUrl(String link){
        return String.format("http://sh.lianjia.com%s", link);
    }
    ...
    private Map<String, Station> setData(List<String> metroName, List<String> metroLink) {
        ...
        Station shanghai = stations.get("上海体育馆");
